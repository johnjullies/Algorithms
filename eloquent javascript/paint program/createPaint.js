...
// we rig the link to update its href attribute whenever it is focused with the keyboard or the mouse is moved over it.
...
// We’ll need the following helper function, which tries to load an image file from a URL and replace the contents of the canvas with it
...
// Apart from the readAsText method we used there, such reader objects also have a method called readAsDataURL, which is exactly what we need here. We load the file that the user chose as a data URL and pass it to loadImageURL to put it into the canvas
...
// Loading a file from a URL is even simpler. But with a text field, it is less clear when the user has finished writing the URL, so we can’t simply listen for "change" events. Instead, we will wrap the field in a form and respond when the form is submitted, either because the user pressed Enter or because they clicked the load button.
